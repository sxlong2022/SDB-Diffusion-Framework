...
def denormalize_bathymetry(normalized_data: np.ndarray, stats: Dict[str, float]) -> np.ndarray:
    """反标准化水深数据"""
    try:
        # 识别陆地区域（使用land_value=1.5的阈值，稍微放宽至0.1）
        land_mask = np.abs(normalized_data - 1.5) < 0.1  # 放宽阈值
        
        # 反标准化：从[-1,1]转换回原始深度范围 (移除 * 2)
        depth_denorm = normalized_data * (stats['std']) + stats['mean']
        ...
        # 3. NaN值设为0
        # 4. 极大值截断 (新增最大深度阈值)
        max_depth_threshold = 100.0  # 例如，设定最大深度为100米，根据实际情况调整
        depth_denorm = np.where(
            np.logical_or(
                land_mask,  # 陆地掩码
                np.logical_or(
                    depth_denorm < 0,  # 负值
                    np.isnan(depth_denorm)  # NaN值
                )
            ),
            0.0,  # 将这些区域设为0
            np.clip(depth_denorm, 0.0, max_depth_threshold) # 截断极大值
        )
